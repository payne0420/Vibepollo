...
namespace platf::dxgi {
  ...
  ipc_session_t::~ipc_session_t() {
    ...
    try {
      ...
      //
      // 1. Stop the control pipe. Resetting the AsyncNamedPipe joins its worker
      ...
      // 2. Terminate the helper process and wait for it to exit so it can no
      //    longer write into the shared texture or the frame metadata view.
      stop_helper_process();

      // 3. Now that every producer is stopped, flush any GPU work on the capture
